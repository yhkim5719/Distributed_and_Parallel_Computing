...
//	MPI_Status status;
	...
	if (rank == 1) {
		number = 2.718281;
		printf ("Process 1 has a new number %f\n\n", number);	
		MPI_Isend (&number, 1, MPI_DOUBLE, 0, 0, MPI_COMM_WORLD, &ireq);
		printf ("Process 1 sent %f to process 0\n", number);
		MPI_Wait(&ireq, &istatus);
	} else if (rank == 0) {
		MPI_Irecv (&number, 1, MPI_DOUBLE, 1, 0, MPI_COMM_WORLD, &ireq);
		MPI_Wait (&ireq, &istatus);
		printf ("Process 0 received %f\n", number);
	}
	printf ("Process %d has number %f\n\n", rank, number);
	end = MPI_Wtime();
